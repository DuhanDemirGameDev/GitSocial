...
function DashboardLayout() {
  ...
  const navItems = [
    { name: 'Akış', path: '/', icon: Home, colorClass: 'text-blue-400', glowClass: 'drop-shadow-[0_0_8px_rgba(96,165,250,0.8)]' },
    { name: 'İş İlanları', path: '/jobs', icon: Briefcase, colorClass: 'text-purple-400', glowClass: 'drop-shadow-[0_0_8px_rgba(192,132,252,0.8)]' },
    { name: 'Topluluklar', path: '/communities', icon: Users, colorClass: 'text-emerald-400', glowClass: 'drop-shadow-[0_0_8px_rgba(52,211,153,0.8)]' },
    { name: 'Profilim', path: '/profile', icon: UserCircle, colorClass: 'text-amber-400', glowClass: 'drop-shadow-[0_0_8px_rgba(251,191,36,0.8)]' },
  ];
  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(async () => {
      ...
      try {
        ...
        console.error("Arama esnasında hata:", err);
      } finally {
        setLoading(false);
      }
    }, 300);
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col relative">
            ...
            <div className="absolute top-full left-8 right-8 mt-2 bg-gray-800/95 backdrop-blur-xl border border-gray-700/70 rounded-2xl shadow-2xl overflow-hidden z-50 max-h-96 overflow-y-auto p-2 space-y-3 custom-scrollbar">
              ...
              {results.users.length > 0 && (
                <div>
                  ...
                  <div className="space-y-1 mt-1">
                    {results.users.map((u) => (
                      <div key={u.id} onClick={() => { setShowResults(false); navigate(`/profile/${u.id}`); }} className="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-gray-700/50 cursor-pointer transition-all">
                        ...
                        {u.accountType === 'COMPANY' && <span className="ml-auto text-[10px] font-extrabold bg-blue-500/10 text-blue-400 px-2 py-0.5 rounded-md border border-blue-500/20">ŞİRKET</span>}
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-1 mt-1">
                    {results.communities.map((c) => (
                      <div key={c.id} onClick={() => { setShowResults(false); navigate(`/communities/${c.id}`); }} className="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-gray-700/50 cursor-pointer transition-all">
                        <div className="w-8 h-8 rounded-xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-400 font-bold text-xs">👥</div>
                        <div className="min-w-0">
              ...
              {results.posts.length > 0 && (
                <div>
                  ...
                  <div className="space-y-1 mt-1">
                    {results.posts.map((p) => (
                      <div key={p.id} onClick={() => { setShowResults(false); navigate(`/post/${p.id}`); }} className="px-3 py-2 rounded-xl hover:bg-gray-700/50 cursor-pointer transition-all block text-left">
              ...
              {results.users.length === 0 && results.posts.length === 0 && results.communities.length === 0 && (
                <div className="text-center py-6 text-sm text-gray-500">Uyuşan bir sonuç bulunamadı 🔍</div>
              )}
            </div>
          )}
        </div>
      ...
      {/* 2. ANA İÇERİK */}
      <main className="flex-1 overflow-y-auto pb-48 p-4 md:p-8">
          <div className="max-w-2xl mx-auto">
              <Outlet />
          </div>
      </main>

      {/* 3. ALT YÜZER MENÜ (GÜNCELLENEN KISIM) */}
      <nav className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50">
        <div className="flex items-center gap-2 bg-gray-800/80 backdrop-blur-xl border border-gray-700/50 p-2 rounded-2xl shadow-2xl">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                to={item.path}
                className={`group relative flex items-center justify-center w-14 h-14 rounded-xl transition-all duration-300 active:scale-90 ${
                  isActive 
                    ? 'bg-gray-700/80 shadow-inner border border-gray-600/50' // Etrafındaki kutu sadece aktifse kalır
              ...
              >
                ...
                {/* İkonlar artık hep kendi renginde parlıyor! */}
                <item.icon 
                  strokeWidth={isActive ? 2.5 : 2} 
                  className={`w-6 h-6 transition-all duration-300 ${item.colorClass} ${
                    isActive 
                      ? `${item.glowClass} -translate-y-1 scale-110` // Aktifse havada kalır ve parlar
                      : `group-hover:${item.glowClass} group-hover:-translate-y-1 group-hover:scale-110` // İnaktifse yerinde durur ama rengi canlı kalır
                  }`} 
                />
              </Link>
            );
          ...
          <button
            ...
            className="group relative flex items-center justify-center w-14 h-14 rounded-xl hover:bg-red-500/10 transition-all active:scale-90 disabled:opacity-50 border border-transparent"
          >
            ...
            <LogOut 
                strokeWidth={2}
                className="w-6 h-6 text-red-400 group-hover:text-red-500 transition-all duration-300 group-hover:-translate-y-1 group-hover:scale-110 group-hover:drop-shadow-[0_0_8px_rgba(239,68,68,0.8)]" 
            />
          </button>
        </div>
      </nav>
